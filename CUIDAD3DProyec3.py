...
def draw_camera_ui(frame, w, h, tvec):
    ...
    # Mostrar datos del Lente usando FONT_HERSHEY_PLAIN (Fijado)
    mag = 1.0 if dist >= 0.45 else 1.0 + (0.45 - dist) * 4.5
    cv2.putText(frame, f"DIST CAM: {dist:.2f}m", (50, 70), cv2.FONT_HERSHEY_PLAIN, 1.0, color, 1, cv2.LINE_AA)
    cv2.putText(frame, f"LENS MAG: {mag:.2f}x", (50, 90), cv2.FONT_HERSHEY_PLAIN, 1.0, color, 1, cv2.LINE_AA)
